...
void sage_reset_global_variables(){
	/* FIXME
  find_all=FALSE; // only used in main and print
  min_weight=0; // used in print_search, set by parse_opts (always 0?)
  min_weight_set=FALSE;
  max_weight=0; // same as min_weight
  max_weight_set=FALSE;
  */
  maximal=FALSE; // reachable from read_options
   /*
  unweighted=FALSE;
  // number1=TRUE; unneeded
  quiet=0;
  only_weight=FALSE;
    */
  sage_clique_count=0;
...
int sage_clique_max(graph_t *g,int **list){
  ...
  // find_all=FALSE; // hmmm
  // maximal=TRUE;
  // number1=FALSE; // unneeded, recheck?
  set_t s;
  int i,l;
